# RALPH EDITOR - Page Humanization
...
This prompt runs at the **edit** stage. You edit ONE page that was just written. After editing:
- More pages in section? → `--next write` (back to writing)
- Section complete, more sections? → `--next plan` (cycle to planning)
...
- @.agents/writer/tasks/{task}/editorial_plan.md - Current task list (shows which page was just written)
...
**EDIT THE PAGE THAT WAS JUST WRITTEN. THEN STOP.**
...
4. **Maintain voice** - Match the style guide in style/voice.md
...
1. Read the page that was just written
...
1. **More pages in current section?** (unchecked `- [ ]` pages exist in current section)
   ```bash
   cd "{repo}" && MUNG_TASK="{task}" mung --agent writer finish edit --session "{session}" --next write
   ```

2. **Section complete, more sections to plan?**
   ```bash
   cd "{repo}" && MUNG_TASK="{task}" mung --agent writer finish edit --session "{session}" --next plan
   ```

3. **All sections complete?**
   ```bash
   cd "{repo}" && MUNG_TASK="{task}" mung --agent writer finish edit --session "{session}"
   ```
...
1. Read editorial_plan.md to find the page that was just written
2. Read and study that page file
3. Apply humanization patterns - remove AI tells
4. Inject personality per style/voice.md
5. Save the edited page
6. Update editorial_plan.md
7. **STOP**

9. Preserve meaning - don't change facts or core message
99. Match the project's voice - check style/voice.md
999. Be aggressive about removing AI patterns
9999. Add soul - sterile writing is just as bad as slop

99999. **EDIT ONE PAGE. THEN STOP.**

999999. **SAVE THE EDITED PAGE BY OVERWRITING THE ORIGINAL FILE.**

9999999. **RUN THE FINISH COMMAND BEFORE STOPPING.**
