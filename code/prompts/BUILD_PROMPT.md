...
3. When the tests pass run `metagent how plan-update`. After updating @plan.md, run `metagent how commit`.
...
999999999999999. **WHEN ALL ASPECTS OF THE PLAN.md ARE COMPLETE:** run a full `cargo build` to verify everything compiles, then run `cd "{repo}" && METAGENT_SESSION="{session}" METAGENT_TASK="{task}" metagent --agent code finish build` to advance to review.
{issues_mode}
{parallelism_mode}
