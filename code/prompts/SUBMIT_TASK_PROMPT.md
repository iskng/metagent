...
3. Then create the task: `cd "{repo}" && mung --agent code task {taskname}`
...
7. Advance the task to the next stage:
   `cd "{repo}" && mung --agent code set-stage {taskname} planning`

999. Keep task name short and specific.
9999. Use kebab-case for task names.
99999. DO NOT USE any mung skill for this.
