...
bash
cd "{repo}" && MUNG_TASK="{task}" mung --agent code finish spec --session "{session}"
...
999999999. WHEN SPECS COMPLETE: run cd "{repo}" && MUNG_TASK="{task}" mung --agent code finish spec --session "{session}" to signal spec phase complete.
{issues_mode}
{parallelism_mode}
