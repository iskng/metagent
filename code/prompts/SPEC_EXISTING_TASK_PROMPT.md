...
4. Task already exists: {task}. Do NOT create a new task. Use `.agents/code/tasks/{task}/spec/` for specs.
...
bash
cd "{repo}" && METAGENT_TASK="{task}" mung --agent code finish spec --session "{session}"

999. Confirm understanding with user before proceeding.
...
999999999. WHEN SPECS COMPLETE: run cd "{repo}" && METAGENT_TASK="{task}" mung --agent code finish spec --session "{session}" to signal spec phase complete.
{issues_mode}
{parallelism_mode}
