...
## Setup
- Run `metagent install` to copy the binary to `~/.local/bin/metagent` and install prompts into `~/.metagent/code/`.
...
## Core workflow
- Initialize: `metagent init [path]`.
- Start interview/spec/planning: `metagent start` (stops at build handoff).
- Create task: `metagent task <name>`.
- Run a task loop: `metagent run <name>`.
- Process queue: `metagent run-queue`.

## Stages and finish
- Stages: `spec -> spec-review -> planning -> build -> review -> completed`.
- Advance a session: `metagent finish <stage> --session <id> [--next <stage>] [--task <name>]`.
...
- Add existing task dir to state: `metagent queue <task>` (creates `task.json` if missing).
- Remove task + files: `metagent dequeue <task>`.
...
- Add: `metagent issue add --title "..." [--task <name>] [--priority P0..P3] [--type spec|build|bug|test|perf|other] [--source review|debug|submit|manual] [--file <path>] [--stage <stage>] [--body <text>|--stdin-body]`.
- Resolve: `metagent issue resolve <id> [--resolution "..."]`.
- Assign: `metagent issue assign <id> --task <name> [--stage <stage>]`.
- Show raw issue file: `metagent issue show <id>`.
...
## Review and debug
- Review: `metagent review <task> [focus text]` (injects focus; stage changes only via `finish`).
- Spec review: `metagent spec-review <task>` (one-shot; stage changes only via `finish`).
- Debug: `metagent debug [--file <path>|--stdin] [bug text...]` (always uses codex and prepends bug context).

## Files and state
- Prompts: `~/.metagent/code/` (used if present; otherwise embedded defaults).
